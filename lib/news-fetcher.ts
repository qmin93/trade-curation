...
function dedupeByUrlAndTitle(items: UnifiedNewsItem[]): UnifiedNewsItem[] {
  ...
  return items.filter((item) => {
    ...
    // 매체별 표현이 달라도 핵심 토큰이 겹치면 같은 스토리 → 제거
...
/** 깨진/불일치 카드 차단: ① 제목 끝 단어가 잘림(댕글링 1글자) ② 헤드라인 종목이 요약에 하나도 없음. */
