...
## B-1: 소스 확보 (발굴/참고용 채널 — 출처로 쓰지 말 것)

⚠️ 아래 텔레그램 채널은 **발굴(참고)용**일 뿐, sourceUrl로 쓰지 않는다. 항상 **원본 기사**를 찾아 출처로 쓰고 요약은 직접 작성한다.

- 1차: 파트 A에서 이미 모은 채널 기사 URL 재사용 (검증 끝난 직링크).
- 2차: 아래 발굴 채널을 WebFetch `https://t.me/s/<채널>`로 읽어 최근 게시물 수집 (증시·종목·테마만):

| 채널 (t.me/s/...) | 외부 기사 링크 | 발굴 방식 |
|---|---|---|
| `FastStockNews` | ✅ 자주 | 게시물의 원본 기사 URL 그대로 사용 |
| `realtime_stock_news` | ✅ 거의 매번 | 원본 기사 URL 그대로 |
| `stock_messenger` | ✅ 자주(네이버) | 원본 기사 URL 그대로 |
| `YeouidoStory2` | ✅ 자주(디일렉 등) | 원본 기사 URL 그대로 |
| `jeilstock` | △ 가끔 | 링크 있으면 그대로, 없으면 종목/이벤트 → WebSearch로 정식 기사 |
| `slav_insight` | △ 가끔 | 위와 동일 |
| `tazastock` | ❌ 거의 없음 | 종목/이벤트(상한가·급등주 리스트 등) 추출 → WebSearch로 정식 기사 확보 |

- 링크 없는 채널은 "오늘 뭐가 떴나(상한가·테마)" 발굴에만 쓰고, **원본 기사는 WebSearch로 직접 찾아 검증**한다. 못 찾으면 제외(B-3·핵심원칙 4).
- 채널 본문(텔방 텍스트)을 그대로 요약에 복붙하지 말 것 — 반드시 원본 기사 본문 기반으로 직접 작성.
