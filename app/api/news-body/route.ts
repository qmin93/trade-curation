...
const TONE: Record<string, string> = {
  ...
  스캘퍼:
    "시초가 단발 스캘퍼지만 정보량은 충분히. '[MM/DD]' + '🔻 이슈' / '📍 자리' 포맷. 직설·속도감·★존댓말. ★길이 제한 없음 — 🔻 이슈와 📍 자리 아래에 핵심 이슈·대응 자리를 정확하고 쉽게 여러 줄로 충분히 풀어준다. 짧게 끊지 말 것(단 군더더기·중복은 빼고 정보 밀도는 높게). 마무리 '시초가 어디일까요?' 류 짧은 존댓말 의문.",
};
...
const CLOSE: Record<string, string> = {
  ...
  스캘퍼: "반드시 '시초가 어디일까요?' 류 짧은 존댓말 의문으로 끝낸다.",
};
...
function systemPrompt(persona: string, fmt: string, withCTA: boolean, withDisc: boolean): string {
  ...
  // 스캘퍼는 정본(persona-voice)상 '길이 제한 없음·충분히'. 나머지는 5줄 압축.
  const lengthRule =
    persona === "스캘퍼"
...
- ★존댓말: 모든 페르소나 존댓말. 반말 금지(스캘퍼도 '시초가 어디일까요?' 류 존댓말 의문형). 단 시그널 대시 불릿의 명사형 종결·데일리 번호의 격식 평서체는 허용.
- 관찰·해설 톤. 매수·매도 '지시'·수익 '보장'·세력/리딩 어투 금지.
- 종목명·수치(%·원·억)는 기사에 있는 것만 정확히. 새 숫자 지어내기 금지.
${lengthRule}${
    withDisc ? `\n- 마지막 줄에 면책 "${DISC}" 을 그대로 넣는다.` : ""
  }${
...
export async function POST(req: Request) {
  let b: {
    ...
    stocks?: string[]; mmdd?: string; withCTA?: boolean; withDisc?: boolean; variant?: number;
  };
  try {
    b = await req.json();
  } catch {
  ...
  try {
    ...
    body = cleaned.join("\n").trim();
    // ★ 5줄 안전망: 모델이 길게 뽑으면 앞 4줄 + 마지막 줄만 유지. (스캘퍼는 길이 제한 없음 — 예외)
    if (persona !== "스캘퍼") {
      const nonEmpty = body.split("\n").filter((l) => l.trim());
      if (nonEmpty.length > 5) {
        body = [...nonEmpty.slice(0, 4), nonEmpty[nonEmpty.length - 1]].join("\n");
      }
    }
    if ((b.withDisc ?? false) && !body.includes("매수·매도 추천 아님")) body += `\n\n${DISC}`;
